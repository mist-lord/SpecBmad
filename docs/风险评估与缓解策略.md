...
- 冒烟与门禁：以四条 CLI 冒烟用例作为质量门槛，校验产物关键字段；CI 门禁覆盖 lint/tsc/test/perf/security，其中 perf 门禁采用 `avgMs` 与 `successRate` 阈值（默认 `MAX_AVG_MS=8000`、`MIN_SUCCESS_RATE=0.95`），security 门禁审计依赖（拒绝 critical/high）
